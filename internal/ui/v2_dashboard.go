...
.ds-stat:hover{background:#141725}
.ds-stat-label{display:flex;align-items:center;gap:7px;font:600 12px 'Hanken Grotesk',sans-serif;color:#9aa0b2;margin-bottom:8px}
.ds-stat-dot{width:8px;height:8px;border-radius:50%}
.ds-stat-value{font:700 36px/1 'Hanken Grotesk',sans-serif}
...
<div class="ds-stats-row">
  <a href="/v2/health" class="ds-stat">
    <div class="ds-stat-label"><span class="ds-stat-dot" style="background:` + healthColor + `"></span>Platform health</div>
    <div class="ds-stat-value" style="color:` + healthColor + `">` + healthPct + `<span style="font:600 16px 'Hanken Grotesk',sans-serif;color:#6b7184">%</span></div>
    <div class="ds-stat-sub">` + fmt.Sprintf("%d/%d", view.HealthyCount, totalStatuses) + ` components</div>
  </a>
  <a href="/v2/timeline" class="ds-stat">
    <div class="ds-stat-label"><span class="ds-stat-dot" style="background:#f5921e"></span>Active threats</div>
    <div class="ds-stat-value" style="color:#f5a443">` + fmt.Sprintf("%d", activeThreats) + `<span style="font:500 12px 'JetBrains Mono',monospace;color:#6b7184;margin-left:6px">origins</span></div>
    <div class="ds-stat-sub">` + html.EscapeString(countryLine) + `</div>
  </a>
  <a href="/v2/providers" class="ds-stat">
    <div class="ds-stat-label"><span class="ds-stat-dot" style="background:#7c6cf2"></span>Blocked</div>
    <div class="ds-stat-value" style="color:#eef0f6">` + fmt.Sprintf("%d", blocked24h) + `</div>
    <div class="ds-stat-sub">` + fmt.Sprintf("%d reported", reported) + `</div>
  </a>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
  ...
  <div class="v2-card">
    ...
    <div style="padding:8px">` + tailHTML.String() + `</div>
  </div>
</div>
...
<script src="/v2/static/attack-map.js"></script>`
	...
	return v2Page("Dashboard", "/v2/", content)
}
