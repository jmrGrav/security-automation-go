...
// writes: OPENAI_API_KEY=test_api_key_value_abc to the file
```
...
key := strings.TrimSpace(string(data))
return key, nil
// returns: "OPENAI_API_KEY=test_api_key_value_abc" — entire line including prefix
```

The provider would call the OpenAI API with `Authorization: Bearer OPENAI_API_KEY=test_api_key_value_abc` which will fail with 401.

**Provider admin UI** (`provider_admin_handlers.go:112`):
```go
writeProviderSecret(secretFile, secret)
// atomicWriteFile(path, []byte(secret), 0o600) — writes raw "test_api_key_value_abc"
```
...
```bash
echo -n "test_api_key_value_abc" | sudo tee /etc/security-automation-go/secrets/openai_api_key
